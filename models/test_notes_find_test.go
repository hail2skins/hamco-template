...
func TestNotesFind(t *testing.T) {
	...
	// Test note finding
	foundNote, err := NotesFind(uint64(note.ID))
	assert.NoError(t, err, "Finding note should not return an error")
	...
	_, err = NotesFind(nonExistentID)
	assert.Error(t, err, "Finding non-existent note should return an error")
